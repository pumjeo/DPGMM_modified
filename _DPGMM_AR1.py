...
def _expectation_corr(varpi, vartheta):
    ...
    return zeta_mean, zeta_squared_mean

class DPGMM_AR1(DPGMM_base):
    ...
    corr_precision_prior : 
        The prior of the precision parameter of the correlation
        distributions (Truncated Normal). If it is None, it's set to 10**-10.
    ...
    varpi_ : array-like of shape (n_components,)
        The mean parameter of the posterior distribution of the correlation (Truncated Normal)
    
    vartheta_ : array-like of shape (n_components,)
        The variance parameter of the posterior distribution of the correlation (Truncated Normal)
    ...
    corr_precision_prior_ : 
        The prior of the precision parameter of the correlation
        distributions (Truncated Normal).
    ...
    _parameter_constraints: dict = {
        **DPGMM_base._parameter_constraints,
        ...
        "corr_precision_prior": [None, Interval(Real, 0.0, None, closed="neither")],
    ...
    def __init__(
        ...
        *,
        ...
        self.corr_precision_prior = corr_precision_prior
    ...
    def _check_corr_parameters(self):
        """Check the prior parameters of the correlation distribution."""
        
        if self.corr_precision_prior is None:
            self.corr_precision_prior_ = 10**-10
        elif self.corr_precision_prior > 0:
            self.corr_precision_prior_ = self.corr_precision_prior
        else:
            raise ValueError(
                "The parameter 'corr_precision_prior' "
                "should be greater than %d, but got %.3f."
                % (0, self.corr_precision_prior)
            )     
    ...
    def _initialize(self, X, y, counts, resp):
        ...
        self.varpi_ = np.repeat(0.5, self.n_components) # Random Initialization
        self.vartheta_ = np.repeat(0.1, self.n_components) # Random Initialization
        
        nk, nk2, xk, sk, tk = self._estimate_gaussian_parameters(X, y, counts, resp)
    ...
    def _estimate_corr(self, X, y, counts, resp):
        """Estimate the precision parameters of the correlation distribution"""
        ...
        resp_duple_one = np.repeat(resp, counts-1, axis=0)
        
        X_1 = np.vstack([mat[1:, :] for mat in X_split])
        X_mi = np.vstack([mat[:-1, :] for mat in X_split])
        y_1 = np.concatenate([mat[1:] for mat in y_split])
        y_mi = np.concatenate([mat[:-1] for mat in y_split])

        tr_theta = np.empty((n_samples-n_subgroups, self.n_components))
        tr_pi = np.empty((n_samples-n_subgroups, self.n_components))
        error_theta = np.empty((n_samples-n_subgroups, self.n_components))
        error_pi = np.empty((n_samples-n_subgroups, self.n_components))
        
        for k in range(self.n_components):
            tr_theta[:,k] = np.einsum('ij,ij->i', X_mi, np.dot(X_mi, self.beta_covariance_[k]))
            tr_pi[:,k] = np.einsum('ij,ij->i', X_1, np.dot(X_mi, self.beta_covariance_[k]))
            error_theta[:,k] = (y_mi - np.dot(X_mi, self.beta_star_mean_[k]))**2
            error_pi[:,k] = (y_mi - np.dot(X_mi, self.beta_star_mean_[k])) * (y_1 - np.dot(X_1, self.beta_star_mean_[k]))
        
        tr_theta_res = (resp_duple_one*tr_theta).sum(axis=0) 
        tr_pi_res = (resp_duple_one*tr_pi).sum(axis=0) 
        error_theta_res = (self.precision_shape_/self.precision_rate_) * (resp_duple_one*error_theta).sum(axis=0)
        error_pi_res = (self.precision_shape_/self.precision_rate_) * (resp_duple_one*error_pi).sum(axis=0)
        
        self.vartheta_ = 1/(self.corr_precision_prior_ + error_theta_res + tr_theta_res)
        self.varpi_ = self.vartheta_ * (error_pi_res + tr_pi_res)
    ...
    def _m_step(self, X, y, counts, log_resp):
        ...
        nk, nk2, xk, sk, tk = self._estimate_gaussian_parameters(
            X, y, counts, np.exp(log_resp)
        )
        ...
        self._estimate_corr(X, y, counts, np.exp(log_resp))
        
    def _estimate_gaussian_parameters(self, X, y, counts, resp):
        ...
        zeta_mean, zeta_squared_mean = _expectation_corr(self.varpi_, self.vartheta_)
        resp_duple_one = np.repeat(resp, counts-1, axis=0)
        ...
        nk = np.dot(counts-1,resp) + 10 * np.finfo(resp.dtype).eps
        nk2 = resp.sum(axis=0) + 10 * np.finfo(resp.dtype).eps

        xk = np.zeros((self.n_components, n_features))
        for k in range(self.n_components):
            xk[k] = (
                     np.sum(resp_duple_one[:,k][:,np.newaxis] * X_1 * y_1[:,np.newaxis], axis=0)
                     - zeta_mean[k] * np.sum(resp_duple_one[:,k][:,np.newaxis] * X_mi * y_1[:,np.newaxis], axis=0)
                     - zeta_mean[k] * np.sum(resp_duple_one[:,k][:,np.newaxis]  * X_1 * y_mi[:,np.newaxis], axis=0)
                     + zeta_squared_mean[k] * np.sum(resp_duple_one[:,k][:,np.newaxis] * X_mi * y_mi[:,np.newaxis], axis=0)
                     )

        sk = np.empty((self.n_components, n_features, n_features))
        for k in range(self.n_components): 
            sk[k] = (
                     np.dot(resp_duple_one[:, k] * X_1.T, X_1) 
                     - zeta_mean[k] * np.dot(resp_duple_one[:, k] * X_mi.T, X_1)
                     - zeta_mean[k] * np.dot(resp_duple_one[:, k] * X_1.T, X_mi)
                     + zeta_squared_mean[k] * np.dot(resp_duple_one[:, k] * X_mi.T, X_mi)
                     )
            sk[k].flat[:: n_features + 1] += self.reg_covar 

        tk = (
              np.dot(resp_duple_one.T, y_1**2)
              - 2 * zeta_mean * np.dot( resp_duple_one.T, y_mi*y_1 )
              + zeta_squared_mean * np.dot( resp_duple_one.T, y_mi**2 )
              )
    ...
    def _estimate_log_prob(self, X, y, counts):
        ...
        log_gauss = np.empty((n_samples-n_subgroups, self.n_components))        
        
        for k in range(self.n_components): 
            first = (self.precision_shape_[k] / self.precision_rate_[k]) * (
                                     (y_1-np.dot(X_1, self.beta_star_mean_[k]))**2 + 
                                      zeta_squared_mean[k] * (y_mi - np.dot(X_mi, self.beta_star_mean_[k]))**2 
                                      - 2 * zeta_mean[k] * (y_1 - np.dot(X_1, self.beta_star_mean_[k]))
                                          * (y_mi - np.dot(X_mi, self.beta_star_mean_[k]))
                                     )
            second = (
                      np.einsum('ij,ij->i', X_1, np.dot(X_1, self.beta_covariance_[k])) 
                      - zeta_mean[k] * np.einsum('ij,ij->i', X_mi, np.dot(X_1, self.beta_covariance_[k]))
                      - zeta_mean[k] * np.einsum('ij,ij->i', X_1, np.dot(X_mi, self.beta_covariance_[k]))
                      + zeta_squared_mean[k] * np.einsum('ij,ij->i', X_mi, np.dot(X_mi, self.beta_covariance_[k]))
                     )
        ...
        temp_split = np.split(temp, np.cumsum(counts-1)[:-1], axis=0)
        log_prob = np.vstack([np.sum(ele, axis=0) for ele in temp_split])
